import sqlite3
from flask import Flask, render_template, request, redirect, url_for, g, flash, session, abort, send_from_directory
...
# PENTING: Ganti dengan kunci rahasia Anda yang sebenarnya dan kuat!
...
def load_logged_in_user():
    ...
    if user_id is not None:
        ...
        if user_data and user_data['is_active'] == 1:
             g.user = user_data
        elif user_data and user_data['is_active'] == 0:
            session.clear() 
...
def index():
    ...
    # --- PENGAMBILAN DATA STATISTIK BARU ---
    total_buku = db.execute("SELECT COUNT(book_id) FROM books").fetchone()[0] or 0
    total_anggota = db.execute("SELECT COUNT(user_id) FROM users WHERE role = 'user'").fetchone()[0] or 0
    ...
    # Asumsi status 'diunduh' untuk peminjaman digital (sesuaikan jika statusnya berbeda)
    ...
    # --- AKHIR PENGAMBILAN DATA STATISTIK BARU ---
    ...
    return render_template('index.html', 
                           ...
                           # --- AKHIR PENGIRIMAN DATA STATISTIK ---
...
def register():
    ...
    if request.method == 'POST':
        ...
        no_telepon = request.form.get('nomor_telepon', '').strip() # Sudah diperbaiki
        ...
        print(f"No Telepon (dari form 'nomor_telepon'): '{no_telepon}'")
        # --- END DEBUG PRINT ---
        ...
        if not error:
            try:
                # --- DEBUG PRINT ---
                print(f"Akan INSERT ke DB: alamat='{alamat if alamat else None}', no_telepon='{no_telepon if no_telepon else None}'")
                # --- END DEBUG PRINT ---
                db.execute(
                    """INSERT INTO users (nama_lengkap, email, password, nomor_anggota, 
                ...
                print(f"SQLite Error on INSERT: {e}") # Tambahan print error SQLite
        if error: flash(error, 'danger')
    return render_template('register.html', title="Registrasi Anggota", now=datetime.datetime.now())
...
def login():
    ...
    if request.method == 'POST':
        ...
        flash(error, 'danger')
    return render_template('login.html', title="Login Anggota", now=datetime.datetime.now())
...
def dashboard():
    ...
    for loan in loan_history:
        ...
        if loan['tipe_pinjaman'] == 'fisik' and loan['status_pinjaman'] == 'dipinjam' and loan['tanggal_jatuh_tempo']:
            try:
                due_date_str = loan['tanggal_jatuh_tempo'].split(' ')[0]
                due_date = datetime.datetime.strptime(due_date_str, '%Y-%m-%d').date()
                days_until_due = (due_date - today).days
                if days_until_due < 0: 
                    overdue_books.append({
                        'judul': loan['judul_buku'], 'jatuh_tempo': due_date_str,
            ...
            except (ValueError, TypeError) as e: print(f"Error parsing date for loan_id {loan['loan_id']}: {e}")
    return render_template('dashboard.html', 
                           title="Dashboard Anggota", loan_history=loan_history, 
                           total_unpaid_fines=total_unpaid_fines,
                           upcoming_due_books=upcoming_due_books, overdue_books=overdue_books, now=datetime.datetime.now())
...
def member_card():
    if not g.user:
        ...
        return redirect(url_for('dashboard'))
    return render_template('member_card.html', title="Kartu Anggota Digital", now=datetime.datetime.now())
...
def edit_profile():
    ...
    if request.method == 'POST':
        ...
        no_telepon_baru = request.form.get('nomor_telepon', '').strip() # Sudah diperbaiki
        ...
        print(f"Form Data: {request.form}")
        print(f"Nama Lengkap: '{nama_lengkap}'")
        print(f"Email Baru: '{email_baru}'")
        print(f"Alamat Baru: '{alamat_baru}'")
        print(f"No Telepon Baru (dari form 'nomor_telepon'): '{no_telepon_baru}'")
        # --- END DEBUG PRINT ---
        ...
        if error is None:
            try:
                # --- DEBUG PRINT ---
                print(f"Akan UPDATE ke DB: alamat_baru='{alamat_baru if alamat_baru else None}', no_telepon_baru='{no_telepon_baru if no_telepon_baru else None}'")
                # --- END DEBUG PRINT ---
                db.execute(
                    """UPDATE users SET nama_lengkap = ?, email = ?, password = ?, 
                       foto_profil_path = ?, alamat = ?, no_telepon = ?, updated_at = datetime('now')
                       WHERE user_id = ?""",
                ...
                g.user = db.execute('SELECT * FROM users WHERE user_id = ?', (g.user['user_id'],)).fetchone() # Muat ulang data g.user
                return redirect(url_for('dashboard')) 
            except sqlite3.Error as e: 
                error = f"Gagal memperbarui profil: {e}"
                print(f"SQLite Error on UPDATE: {e}") # Tambahan print error SQLite
        
        if error: flash(error, "danger")
    
    current_user_data_for_form = dict(g.user) if g.user else {}
    if request.method == 'POST' and error: 
        current_user_data_for_form['nama_lengkap'] = request.form.get('nama_lengkap', g.user.get('nama_lengkap', '') if g.user else '')
        current_user_data_for_form['email'] = request.form.get('email', g.user.get('email', '') if g.user else '')
        current_user_data_for_form['alamat'] = request.form.get('alamat', g.user.get('alamat', '') if g.user else '')
        current_user_data_for_form['no_telepon'] = request.form.get('nomor_telepon', g.user.get('no_telepon', '') if g.user else '') # Sudah diperbaiki
    
    return render_template('edit_profile.html', title="Edit Profil", user_data=current_user_data_for_form, now=datetime.datetime.now())
...
def books_catalog_physical():
    ...
    return render_template('books_catalog.html', 
...
def book_detail(book_id):
    ...
    if book is None:
        abort(404) 
    return render_template('book_detail.html', title=book['judul'], book=book, now=datetime.datetime.now())
...
def borrow_book(book_id):
    ...
    if book is None:
        ...
        return redirect(url_for('books_catalog_physical'))
    if book['jenis_buku'] == 'digital':
        ...
        try:
            ...
            db.execute(
                """INSERT INTO loans (user_id, book_id, tanggal_pinjam, tipe_pinjaman, status_pinjaman)
                   VALUES (?, ?, ?, ?, ?)""",
                (g.user['user_id'], book_id, tanggal_pinjam, 'digital', 'diunduh')
            )
            db.commit()
        ...
        try:
            tanggal_pinjam = datetime.datetime.now()
            tanggal_jatuh_tempo = tanggal_pinjam + datetime.timedelta(days=7)
            if book['stok_fisik'] > 0:
                existing_loan = db.execute(
                    "SELECT loan_id FROM loans WHERE user_id = ? AND book_id = ? AND status_pinjaman = 'dipinjam'",
                    (g.user['user_id'], book_id)
                ).fetchone()
                if existing_loan:
                    flash(f"Anda sudah meminjam buku fisik '{book['judul']}' dan belum dikembalikan.", "warning")
                    return redirect(url_for('book_detail', book_id=book_id))

                cursor = db.execute( 
                    """INSERT INTO loans (user_id, book_id, tanggal_pinjam, tanggal_jatuh_tempo, tipe_pinjaman, status_pinjaman)
                       VALUES (?, ?, ?, ?, ?, ?)""",
                    (g.user['user_id'], book_id, tanggal_pinjam, tanggal_jatuh_tempo, 'fisik', 'dipinjam')
                )
                db.execute("UPDATE books SET stok_fisik = stok_fisik - 1 WHERE book_id = ?", (book_id,))
                db.commit()
                new_loan_id = cursor.lastrowid 
                flash(f"Anda berhasil meminjam buku fisik '{book['judul']}'. Harap kembalikan sebelum {tanggal_jatuh_tempo.strftime('%d-%m-%Y')}.", "success")
                if new_loan_id:
            ...
            flash(f"Terjadi kesalahan database saat memproses peminjaman buku fisik: {e}", "danger")
        return redirect(url_for('book_detail', book_id=book_id))
    else:
...
def loan_receipt(loan_id):
    ...
    if not loan:
        flash("Struk peminjaman tidak ditemukan atau bukan untuk buku fisik.", "danger")
        return redirect(url_for('dashboard'))
    
    if loan['peminjam_user_id'] != g.user['user_id'] and g.user['role'] != 'admin':
        flash("Anda tidak memiliki izin untuk melihat struk peminjaman ini.", "danger")
        return redirect(url_for('dashboard'))

    return render_template('loan_receipt.html', title="Struk Peminjaman Buku", loan=loan, now=datetime.datetime.now())
...
def admin_dashboard():
    db = get_db()
    total_buku = db.execute("SELECT COUNT(book_id) FROM books").fetchone()[0] or 0
    total_user = db.execute("SELECT COUNT(user_id) FROM users WHERE role = 'user'").fetchone()[0] or 0
    ...
    recent_added_books = db.execute(
        """
        SELECT judul, pengarang, created_at FROM books 
    ...
    return render_template('admin/admin_dashboard.html', 
                           ...
                           recent_added_books=recent_added_books)
...
def add_book():
    ...
    if request.method == 'POST':
        ...
        if error: 
            flash(error, "danger")
            return render_template('admin/add_book.html', title="Tambah Buku Baru", categories=all_categories, now=datetime.datetime.now(), request_form=request.form)
        ...
        if error is None:
            try:
                db.execute(
                    """INSERT INTO books (judul, pengarang, category_id, jenis_buku, ringkasan, file_ebook_path, cover_image_path, stok_fisik, updated_at)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))""",
                    (judul, pengarang, category_id_to_use, jenis_buku, ringkasan, file_ebook_path, cover_image_path, stok_fisik if jenis_buku == 'fisik' else 0)
                )
                db.commit()
...
def admin_list_books():
    ...
    books_data = db.execute(
        """
        SELECT b.book_id, b.judul, b.pengarang, b.jenis_buku, b.stok_fisik, c.nama_kategori
        FROM books b
        LEFT JOIN categories c ON b.category_id = c.category_id
        ORDER BY b.book_id DESC
        """
    ).fetchall()
    return render_template('admin/list_books.html', title="Daftar Semua Buku", books=books_data, now=datetime.datetime.now())

@app.route('/admin/edit_book/<int:book_id>', methods=['GET', 'POST'])
@admin_required
def admin_edit_book(book_id):
    db = get_db()
    book_to_edit = db.execute(
        """
        SELECT b.*, c.nama_kategori 
        FROM books b
        LEFT JOIN categories c ON b.category_id = c.category_id
    ...
    if request.method == 'POST':
        ...
        if category_id_to_use:
             ...
             form_data_for_render['nama_kategori'] = cat_name_row['nama_kategori'] if cat_name_row else book_to_edit['nama_kategori']
        else:
            form_data_for_render['nama_kategori'] = book_to_edit['nama_kategori']
        ...
        if error:
            ...
            return render_template('admin/edit_book.html', title=f"Edit Buku: {book_to_edit['judul']}", book=form_data_for_render, categories=all_categories, now=datetime.datetime.now())
        ...
        if error is None:
            try:
                db.execute(
                    """UPDATE books 
                       SET judul = ?, pengarang = ?, category_id = ?, jenis_buku = ?, 
                           ringkasan = ?, file_ebook_path = ?, cover_image_path = ?, 
                           stok_fisik = ?, updated_at = datetime('now')
                       WHERE book_id = ?""",
                    (judul, pengarang, category_id_to_use, jenis_buku, ringkasan, 
    ...
    return render_template('admin/edit_book.html', title=f"Edit Buku: {book_to_edit['judul']}", book=book_to_edit, categories=all_categories, now=datetime.datetime.now())
...
def admin_active_loans():
    db = get_db()
    active_physical_loans = db.execute(
        """
        SELECT l.loan_id, l.tanggal_pinjam, l.tanggal_jatuh_tempo,
               u.user_id AS peminjam_user_id, u.nama_lengkap AS nama_peminjam, u.nomor_anggota,
               b.judul AS judul_buku, b.book_id
        FROM loans l
        JOIN users u ON l.user_id = u.user_id
        JOIN books b ON l.book_id = b.book_id
        WHERE l.tipe_pinjaman = 'fisik' AND l.status_pinjaman = 'dipinjam'
    ...
    if processed_loan_id:
        processed_loan_details = db.execute(
             """
            SELECT l.*, u.nama_lengkap AS nama_peminjam, u.user_id, b.judul AS judul_buku
            FROM loans l
            JOIN users u ON l.user_id = u.user_id
            JOIN books b ON l.book_id = b.book_id
            WHERE l.loan_id = ?
...
def admin_return_book(loan_id):
    db = get_db()
    loan = db.execute("SELECT * FROM loans WHERE loan_id = ? AND tipe_pinjaman = 'fisik' AND status_pinjaman = 'dipinjam'", (loan_id,)).fetchone()
    if not loan:
    ...
    try:
        if tanggal_jatuh_tempo_str:
            jatuh_tempo = datetime.datetime.strptime(tanggal_jatuh_tempo_str.split(' ')[0], '%Y-%m-%d')
            if tanggal_kembali.date() > jatuh_tempo.date():
                selisih_hari = (tanggal_kembali.date() - jatuh_tempo.date()).days
...
def admin_list_users():
    db = get_db()
    users_data = db.execute("SELECT user_id, nama_lengkap, email, nomor_anggota, role, is_active, created_at, updated_at FROM users ORDER BY user_id DESC").fetchall()
    return render_template('admin/list_users.html', title="Manajemen Pengguna", users=users_data, now=datetime.datetime.now())
...
def admin_toggle_user_role(user_id):
    ...
    if user_to_update['user_id'] == g.user['user_id']:
        ...
        return redirect(url_for('admin_list_users'))
    new_role = 'admin' if user_to_update['role'] == 'user' else 'user'
    try:
        db.execute("UPDATE users SET role = ?, updated_at = datetime('now') WHERE user_id = ?", (new_role, user_id))
        db.commit()
        flash(f"Role untuk pengguna '{user_to_update['nama_lengkap']}' berhasil diubah menjadi '{new_role}'.", "success")
    except sqlite3.Error as e:
...
def admin_toggle_user_active_status(user_id):
    ...
    try:
        db.execute("UPDATE users SET is_active = ?, updated_at = datetime('now') WHERE user_id = ?", (new_status, user_id))
        db.commit()
...
def admin_edit_fine(loan_id):
    ...
    if not user_id_for_back_link and loan_detail: 
    ...
    if request.method == 'POST':
        ...
        if error:
            flash(error, "danger")
        # Selalu render template dengan data saat ini jika ada error atau GET request
        return render_template('admin/edit_fine.html', title="Edit Denda Peminjaman", loan=loan_detail, user_id_for_back=user_id_for_back_link, now=datetime.datetime.now())
